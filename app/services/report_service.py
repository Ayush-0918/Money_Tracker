...
async def get_coach_report(
    ...
    merch_stmt = select(Transaction.merchant, func.sum(Transaction.amount).label("total")).where(
        and_(
            Transaction.user_id == user_id,
            Transaction.transaction_date >= month_start_utc,
            Transaction.transaction_date <= month_end_utc
        )
    ).group_by(Transaction.merchant).order_by(func.sum(Transaction.amount).desc()).limit(3)
    top_merchants = [{"merchant": row.merchant, "spent": float(row.total)} for row in (await db.execute(merch_stmt)).all()]
    ...
    if not insights:
        insights = []
        if spend_increased:
            insights.append(f"You spent {round(spend_diff_pct, 1)}% more than last month. Consider trimming non-essential shopping.")
        else:
            insights.append("Great job! Your spending is lower than last month.")
        
        if top_merchants:
            top_m = top_merchants[0]
            insights.append(f"Your top expense was ₹{top_m['spent']:,} at {top_m['merchant']}. Try reducing transactions here next week.")
        else:
